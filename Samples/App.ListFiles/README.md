...
`[assembly:]` block at the top of `ListFiles.main.cs` is what
...
usage: ListFiles [path] [+<attrs>] [-<attrs>] [-all] [-out[:<file>]]
  ...
  -out      write TSV to a timestamped file under the samples-domain
            Results folder instead of stdout
...
```
dotnet run --project Samples/App.ListFiles -- . +H -R -all
dotnet run --project Samples/App.ListFiles -- . -out
```
...
```
dotnet run --project Samples/App.ListFiles -- . > listing.tsv
```
...
## Assembly identity, not csproj synthesis
...
`[assembly:]` attribute right here in `ListFiles.main.cs`. The file
...
## Traditional arg parsing vs. a command-ops model
...
parsing any .NET console app writes, not a named-command operations
model (commands registered as types, dispatched by a command
router -- the author's `Cmd.OpsBase.CmdApp`, which is not part of
this library's published set). Samples are demonstrations of
individual types cooperating, not of an ops framework -- so
`ListFiles` (and `Lookup`) stick to plain
...
`Config.FolderPath(Config.FolderId.Results)` -- the same domain
Results folder every Samples77NW exe shares via `Config.cs`.
`-out:<file>` takes an exact path instead.
