...
// contract). The domain-wide attributes (Company, Copyright,
// Contact, DomainName, DomainGuid, DeployDebug, ExeInterface) live
// once in samples\Config.cs, linked into every sample exe via
// samples\Directory.Build.props (supersedes SamplesCommon.cs; previously
// duplicated per exe here - see done\JOB.SAMPLES D1/D2/D5).
// AssemblyVersion is likewise no longer hand-written here - it is
// auto-stamped at every build by samples\Directory.Build.props.
// ================================================================
...
namespace Samples77NW
{
    ...
    /// Folder) meant to paste straight into Excel. Boots through
    /// <see cref="Config.Initialized"/> the way every Samples77NW exe
    /// does, then behaves like an ordinary command-line tool from
    ...
    internal static class ListFilesMain
    {
        ...
        private static int Main()
        {
            ...
            // standards (Config.Issues) before anything else runs, so no
            // per-exe Exe.SetCritical call is needed here. False = no
            // domain folder = no consent yet: Permit.Request presents the
            // notice and asks - no Y, no run (JOB.PERMIT 2026-07-29).
        ...
        private static TextWriter _OpenOutput()
        {
            ...
            {
                // D1 (JOB.CONFORM 2026-07-28): bare -out now lands under
                // the domain Results folder, not Exe.DomainFolderPath's
                // UserDocuments - matches FileList's convention.
                _OutFilePath = Config.FolderPath(Config.FolderId.Results)
        ...
        private static void _Usage()
        {
            ...
            Console.Error.WriteLine("  -out      write TSV to a timestamped file under the domain");
            Console.Error.WriteLine("            Results folder instead of stdout");
