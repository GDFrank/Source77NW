...
namespace Source77NW
{
    ...
    public sealed class CryptoKey : IDisposable
    {
        ...
        private CryptoStream _stream_sym(SymmetricAlgorithm x, Stream baseStream, bool as_reader_else_writer)
        {
            ...
            if (key != null) // null-guard â€” embedded XOR/None keys have key/iv == null; SymmetricAlgorithm setters throw ArgumentNullException
                x.Key = key;
        ...
        public void Dispose()
        {
            if (_disposed || IsEmbedded) return; // FIX: Create(None/XOR/Jumble) return shared singletons â€” never mark them disposed
            Clear();
            _disposed = true;
        }
