...
namespace Source77NW
{
    ...
    public enum ResKind : byte
    {
        ...
        blob = 3,
        /// <summary>special blob used by lingoMgr.</summary>
        lingo = 4, 
    }
    ...
    public struct ResCode : IResCode, IComparable<ResCode>
    {
        ...
        /// <summary>
        /// Onetime register of who maintains current LingoNum;
        /// false only when the delegate is null. Thread safe via
        /// copy-on-write.
        /// </summary>
        public static bool RegisterAsLingoNumAuthority(out Action<byte> returnSetLingoNum)
        {
            returnSetLingoNum = _SetLingoNum;

            return true;
        }
        private static void _SetLingoNum(byte theNum) { _ActiveLingoNum = theNum; }
