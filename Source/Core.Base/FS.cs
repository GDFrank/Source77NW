...
namespace Source77NW
{
    ...
    public static class FS
    {
        ...
        /// <see cref="ValidPath_or_null"/>. Actual resolution depends on
        /// <see cref="EnableValidPathSearch"/> - probes drives only when a
        /// search is enabled; degrades to a plain
        /// <see cref="FolderId.SystemDrive"/> substitution otherwise.</summary>
        ...
        // Lazily resolved - see _ExeDriveOrDefault. NOT an eager static
        // initializer on purpose: forcing Exe.ExeFilePath to resolve at
        // FS's very first touch (regardless of whether Exe_drive_1st mode
        // is ever used) would create a startup-order hazard, and a short/
        // odd ExeFilePath (UNC launch, edge timing) throwing INSIDE a
        // static field initializer would poison the whole FS type for the
        // rest of the process (TypeInitializationException on every
        // subsequent FS call, not just the drive-search ones).
        private static string _ExeDrive;

        // Resolves and caches the exe's own drive prefix ("C:\") on first
        // real need (Exe_drive_1st mode actually being used), never
        // eagerly. Falls back to theFallback (the caller's own first
        // candidate, e.g. xDrives[0]) if ExeFilePath is missing, too
        // short, or throws - never lets a resolution failure here bubble
        // up and break drive search entirely.
        private static string _ExeDriveOrDefault(string theFallback)
        {
            if (_ExeDrive == null)
            {
                try
                {
                    string sExePath = Exe.ExeFilePath;
                    _ExeDrive = (sExePath != null && sExePath.Length >= 3) ? sExePath.Substring(0, 3) : theFallback;
                }
                catch
                {
                    _ExeDrive = theFallback;
                }
            }
        ...
        /// <paramref name="issue_if_is_file"/> = false).
        /// </summary>
        ...
        /// <summary>Converts a path to a "*:" path suitable
        /// ValidPath_or_null feature to search all drives for
        /// first existing path or default to the 1st system drive.
        ///
        /// Returns null if:
        /// thePath[1] not equal COLON
        /// thePath[2] not equal to BSLASH OR SLASH
        /// 
        /// </summary>
        public static string AsAnyDrivePath_or_null(string thePath)
        {
            if (string.IsNullOrWhiteSpace(thePath))
                return null; 
            if (thePath.Length < 3)
                return null;
            if (thePath[2] != Chars.BSLASH && thePath[2] != Chars.SLASH) return null;
            if (thePath[1] != Chars.COLON) return null;
            if (thePath[0] == Chars.STAR) return thePath;
            return Chars.STAR + thePath.Substring(1);
        }

        /// <summary> Returns false if:
        /// thePath[0] not equal to STAR
        /// thePath[1] not equal to COLON
        /// thePath[2] not equal to BSLASH or SLASH
        /// otherwise: returns true
        /// </summary>
        public static bool IsAnyDrivePath(string thePath)
        {
            if (string.IsNullOrWhiteSpace(thePath))
                return false; 
            if (thePath.Length < 3)
                return false;
            if (thePath[0] != Chars.STAR) return false;
            if (thePath[2] != Chars.BSLASH && thePath[2] != Chars.SLASH) return false;
            if (thePath[1] != Chars.COLON) return false;
            return true;
        }

        private static ValidPathSearchDO _DO_Search = null;
        private static ValidPathSearchId _ID_Search = 0;

        /// <summary>The built-in <see cref="ValidPathSearchDO"/> for
        /// <see cref="ValidPathSearchId.Mounted_drives"/> and
        /// <see cref="ValidPathSearchId.Exe_drive_1st"/>. PRECONDITION:
        /// only called by <see cref="ValidPath_or_null"/> after it has
        /// already confirmed thePath starts with <see cref="STAR_COLON_DSep"/> -
        /// this method does not re-check.</summary>
        private static string _ValidPathSearch_or_null(string thePath, out Issue returnIssue)
        {
            returnIssue = null;

            bool bExe1st = _ID_Search == ValidPathSearchId.Exe_drive_1st;

            string sPathTail = thePath.Substring(STAR_COLON_DSep.Length);

            string[] xDrives = Environment.GetLogicalDrives();

            string sExeDrive = bExe1st ? _ExeDriveOrDefault(xDrives[0]) : null;

            if (bExe1st && 0 != string.Compare(xDrives[0], sExeDrive, true))
            {
                int i1 = -1;
                while (++i1 < xDrives.Length)
                {
                    if (string.Equals(xDrives[i1], sExeDrive, StringComparison.OrdinalIgnoreCase))
                    {
                        while (i1 > 0)
                            xDrives[i1--] = xDrives[i1];

                        xDrives[i1] = sExeDrive;
                        break;
                    }
                }
            }
            ...
            return bExe1st ? sExeDrive + sPathTail : xDrives[0] + sPathTail;
        }

        public delegate string ValidPathSearchDO(string fromPath, out Issue returnIssue);

        /// <summary>Built-in <see cref="EnableValidPathSearch"/> modes for
        /// the "*:\" any-drive prefix. Neither is active until the
        /// EntryAssembly enables one (or supplies its own
        /// <see cref="ValidPathSearchDO"/>) - see <see cref="ValidPath_or_null"/>'s
        /// no-search default when nothing is enabled at all.</summary>
        public enum ValidPathSearchId : byte
        {
            /// <summary>Probe every logical drive in
            /// <see cref="Environment.GetLogicalDrives"/> order.</summary>
            Mounted_drives = 1,
            /// <summary>Try the running exe's own drive first, then every
            /// other logical drive.</summary>
            Exe_drive_1st = 2,
        }

        /// <summary>Enables the built-in "*:\" drive-search behavior with a
        /// named mode. One-shot - false if a search (named or custom) is
        /// already enabled, or theId isn't a defined
        /// <see cref="ValidPathSearchId"/>.</summary>
        public static bool EnableValidPathSearch(ValidPathSearchId theId)
        {
            if (_DO_Search != null || _ID_Search != 0) return false;

            if (!Enum.IsDefined(typeof(ValidPathSearchId), theId)) return false;

            _DO_Search = _ValidPathSearch_or_null;
            _ID_Search = theId;
            return true;
        }

        /// <summary>Enables "*:\" drive search via the EntryAssembly's own
        /// logic instead of a built-in mode. One-shot - false if a search
        /// (named or custom) is already enabled, or theSearchDO is null.</summary>
        public static bool EnableValidPathSearch(ValidPathSearchDO theSearchDO)
        {
            if (_DO_Search != null || _ID_Search != 0) return false;

            if (theSearchDO == null) return false;

            _DO_Search = theSearchDO;
            return true;
        }

        /// <summary>Validates and normalizes a path. Accepts a rooted path,
        /// a file:// URL (converted, %20 restored; other % escapes are a
        /// BadEntry issue), or an "*:\" prefix. The "*:\" prefix's actual
        /// resolution depends on <see cref="EnableValidPathSearch"/>: with
        /// a search enabled, it probes drives per that mode (see
        /// <see cref="ValidPathSearchId"/>) or the EntryAssembly's own
        /// logic; with nothing enabled, it degrades to a plain
        /// <see cref="FolderId.SystemDrive"/> substitution - no probing.
        /// Null for null/whitespace input WITHOUT an issue (no harm no
        /// foul); null WITH a BadEntry issue for unrooted or invalid
        /// paths.</summary>
        public static string ValidPath_or_null(string thePath_or_AnyDrivePath_or_FileUrl, out Issue returnIssue)
        {
            returnIssue = null;

            if (string.IsNullOrWhiteSpace(thePath_or_AnyDrivePath_or_FileUrl))
                return null; // NO Issue ...no harm no foul

            string sPath1 = thePath_or_AnyDrivePath_or_FileUrl; // as either
            ...
            try
            {
                if (sPath1.StartsWith(STAR_COLON_DSep))
                {
                    if (_DO_Search != null)
                    {
                        string sPathFound = _DO_Search(sPath1, out returnIssue);
                    ...
                    {
                        // No search enabled (the EntryAssembly never called
                        // EnableValidPathSearch) - "*:\" degrades to a plain
                        // SystemDrive substitution, no drive probing at all.
                        sPath1 = FolderPath(FolderId.SystemDrive) + sPath1.Substring(STAR_COLON_DSep.Length);
                    }
                }
                ...
                if (AS.Url.IsFileScheme(sPath1, out string sPrefix, out string sValue))
                {
                    ...
                    int iLen = thePath_or_AnyDrivePath_or_FileUrl.Length - iStart;
                    int iHash = thePath_or_AnyDrivePath_or_FileUrl.IndexOf(Chars.HASH, iStart, iLen);

                    if (iHash > 0)
                    {
                        // JUST IN CASE THE URL contains HASH, like a web url
                        iLen -= (thePath_or_AnyDrivePath_or_FileUrl.Length - iHash);
                        sPath1 = thePath_or_AnyDrivePath_or_FileUrl.Substring(iStart, iLen);
                    }
                    else
                    {
                        sPath1 = thePath_or_AnyDrivePath_or_FileUrl.Substring(iStart);
                    }
        ...
        public enum IssueId : byte
        {
            ...
            Invalid_path = 3,
            Invalid_URL = 4,
            No_files_to_process = 5,
            File_not_found = 6,
            File_could_not_be_created = 7,
            File_must_NOT_exist = 8,
            File_or_Folder_must_exist = 9,
            Source_file_not_found = 10,
            Source_folder_not_found = 11,
            FolderPath_cannot_be_existing_file = 12,
            FilePath_cannot_be_existing_folder = 13,
            Folder_could_not_be_created = 14,
            Folder_must_NOT_exist = 15,
            Folder_not_found = 16,
            Folder_must_be_empty = 17,
            FolderPath_cannot_be_root = 18,
            Folders_must_be_distinct = 19,
            Path_must_have_root_folder = 20,
            Root_folder_operation_not_supported = 21,
            Url_may_not_contain_control_characters = 22,
            Unexpected_file_access_issue = 23,
            Cannot_find_exe_file = 24,
            Nothing_to_search_for = 25,
            Failed_to_start_exe = 26,
            Operation_failed = 27,
            Unable_to_write_to_file = 28,
            Unable_to_access_locked_file = 29,
            Unable_to_write_to_folder = 30,
            Unable_to_write_to_stream = 31,
            Unable_to_open_directory = 32,
            Unable_to_open_file_or_folder = 33,
            Unable_to_load_file = 34,
            Unable_to_load_bitmap = 35,
            Unable_to_set_value = 36,
            Unable_to_create_file = 37,
            Unable_to_delete_file = 38,
            Unable_to_replace_file = 39,
            Unable_to_create_folder = 40,
            Unable_to_delete_folder = 41,
            Unable_to_delete_contents = 42,
            Unable_to_find_marked_text = 43,
            Unable_to_access_restricted_file = 44,
            Unable_to_access_locked_file_or_files = 45,
        }
    }
}
