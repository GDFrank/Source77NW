...
namespace Source77NW
{
    ...
    public sealed class EnumCodes : IComparable<EnumCodes>
    {
        ...
        /// <summary>Binary-searches for theFlag among the enum's ascending flag values. Returns its index with returnFound=true if matched, else returnFound=false and the insertion point (index of the nearest value greater than theFlag, or Count if theFlag exceeds all).</summary>
        public int IndexNearest(ulong theFlag, out bool returnFound)
        {
            int lo = 0, hi = Count - 1;
            while (lo <= hi)
            {
                int mid = lo + ((hi - lo) >> 1);
                ulong midFlag = Flag64(mid);
                if (midFlag == theFlag) { returnFound = true; return mid; }
                if (midFlag < theFlag) lo = mid + 1;
                else hi = mid - 1;
            }
            returnFound = false;
            return lo; // insertion point, same semantics as ~i from Array.BinarySearch
        }
