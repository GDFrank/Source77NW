...
namespace Source77NW
{
    ...
    /// <summary>
    /// The single application exception of Source77NW: carries a
    /// user-facing message plus a forensic trail - an IssueKind, the
    /// raising module's Source number, and a Spot byte pinning the
    /// exact raise site - so dialogs stay humane while logs identify
    /// the origin precisely.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Created only through Create(); constructors are private. The
    /// type is sealed by design: handling dispatches on Kind (data)
    /// rather than on exception subtype (hierarchy) - one exception
    /// type, many kinds - enabling friendly user-facing responses and
    /// hard-core forensic analysis from the same object.
    /// </para>
    /// <para>
    /// By convention each source file declares a <c>const ushort
    /// issueSource</c> identifying that module, and each raise site
    /// within it uses a distinct Spot byte. Source77NW core tiers
    /// reserve issueSource 65,000-65,535.
    /// </para>
    /// <para>
    /// Kind, Source, and Spot pack into Exception.HResult as bits 0-7
    /// Spot, bits 8-23 Source, bits 24-28 Kind. This is a private
    /// scheme: the value is not a COM HRESULT (severity bit 31 is
    /// never set), so it must not be handed to HRESULT-interpreting
    /// APIs.
    /// </para>
    /// <para>
    /// CONTRACT BY SIGNATURE - the library-wide no-throw contract,
    /// documented HERE ONCE (DEV.txt XML DOC STANDARDS governs how
    /// every other member relies on this instead of restating it):
    /// </para>
    /// <list type="bullet">
    /// <item><description><c>T F(..., out Issue returnIssue)</c> -
    /// never throws; the result is valid when returnIssue is
    /// null</description></item>
    /// <item><description><c>bool F(..., out Issue returnIssue)</c> -
    /// never throws; true means issue is null, false means issue is
    /// set</description></item>
    /// <item><description><c>Try*</c> / <c>Did*</c> / <c>Got*</c> -
    /// never throws (mnemonic-explicit)</description></item>
    /// <item><description>everything else - fails LOUD via Issue on
    /// contract misuse</description></item>
    /// </list>
    /// <para>
    /// NEVER THROWS: a member whose summary ends "Never throws." (and
    /// that lacks one of the signature markers above, so the reader
    /// could not already tell) asserts the same no-throw contract by
    /// prose instead of by shape. In both forms the promise is an
    /// ATTEMPT, not a guarantee against catastrophic process-level
    /// exceptions - OutOfMemoryException, StackOverflowException -
    /// which are process death, not contract violations, and are
    /// never what "never throws" means.
    /// </para>
    /// </remarks>
    public sealed class Issue : ApplicationException
    {
        ...
        /// <paramref name="theKinds"/>. Never throws.
        /// </summary>
        ...
        /// (at or above <see cref="IssueKind.ProgramIssue"/>). Never throws.
        /// </summary>
        ...
        /// (or null) maps to <see cref="IssueKind.Unspecified"/>. Never
        /// throws.
        /// </summary>
        /// <remarks>
        ...
        /// for other exceptions: "Issue: MappedKind TypeName". Empty for
        /// null. Never throws.
        /// </summary>
