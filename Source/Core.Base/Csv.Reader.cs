...
namespace Source77NW
{
    ...
    /// opening quote must have a closing quote. Per RFC 4180 section 2: an
    /// escaped "" pair inside a quoted field stays in the raw Value view
    /// as-is (never rewritten) and surfaces via <see cref="ToString"/> as
    /// one literal '"' (rule 7); a CR/LF inside a quoted field is literal
    /// field content, not a record terminator (rule 6) - so a quoted field
    /// may legitimately span more than one physical line. Write-out is
    /// always TSV (<see cref="WriteTsvValues(Csv[], TextWriter)"/>),
    /// quoting a value only when it contains quotes, tabs, or line breaks
    /// (or when forced).
    /// </remarks>
    public struct Csv : IComparable<Csv>, IDisposable
    {
        ...
        public sealed class Reader
        {
            ...
            public void Reset(string theText, bool asHasHeader, char theSeparator_TAB_COMMA_SEMI_else_autodetect)
            {
                ...
                {
                    SepChar = Chars.NUL; // UNTIL Separator is discovered
                }
            ...
            /// set: unclosed quote - the only remaining parse-issue case, per
            /// RFC 4180 a quoted field may legitimately span multiple physical
            /// lines and is no longer an error).
            /// </summary>
            public bool GotRecord()
            {
                ...
                while (!bEndOfRecord) // PARSE-PUSH VALUE
                {
                    ...
                    while (true) // PARSE FIELD CHARS - ONE CHAR PLUCK AT A TIME
                    {
                        ...
                        {
                            // RFC 4180 section 2, rule 7: a quote-char inside a quoted
                            // field is escaped as "" (two quote chars in a row). Peek
                            // the next char without consuming it via BotChar_or_NUL;
                            // a second QUOTE means this pair is literal content, not
                            // the closing quote - consume the second one too and stay
                            // quoted. Both chars stay in the raw span; Value is never
                            // rewritten, ToString() un-escapes "" to one '"' on demand.
                            if (bQuoting && _unparsed.BotChar_or_NUL == Chars.QUOTE)
                            {
                                _unparsed.PluckChar_or_NUL(); // consume the escaping quote's pair-mate

                                continue; // still quoting; "" stays in the raw span for ToString() to unescape
                            }
                        ...
                        if (Chars.CatTrue(iPluckCat, EndOfRecordCats)) // CR / LF
                        {
                            ...
                            // RFC 4180 section 2, rule 6: a CR/LF encountered while still
                            // inside a quoted field is literal field content, not a record
                            // terminator.  Fold a CRLF pair into one consumed unit and continue.
                            if (bQuoting)
                            {
                                if (cBotChar == Chars.CR && _unparsed.BotChar_or_NUL == Chars.LF)
                                {
                                    _unparsed.PluckChar_or_NUL(); // pluck past LF, both stay in the raw span as field content
                                }
        ...
        public static Csv Create(Chars theValue)
        {
            ...
            if (theValue.Length > 1)
            {
                if (theValue.IsQuoted())
                {
                    ...
                    // RFC 4180 rule 7: an escaped "" pair may still be sitting in the
                    // raw span here (the Reader keeps both quote chars together as
                    // literal content rather than treating the second as a closer -
                    // see GotRecord). Value stays this raw zero-copy view always;
                    // ToString() is where "" becomes a single literal '"', driven by
                    // Contains_QUOTE_QUOTE, so the common no-escape field never pays
                    // an allocation.
                }
            }
        ...
        /// <summary>
        /// The value text: the raw view as-is, except when it still contains an
        /// escaped "" pair (<see cref="Contains_QUOTE_QUOTE"/>), in which case
        /// each "" is converted to one literal '"' here (RFC 4180 rule 7). The
        /// common no-escape field costs no allocation; Value itself is never
        /// rewritten - it stays the original zero-copy Chars view.
        /// </summary>
        public override string ToString() => Contains_QUOTE_QUOTE ? Value.ToString().Replace(QUOTE_QUOTE, AS.QUOTE) : Value.ToString();
