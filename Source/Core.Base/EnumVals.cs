...
namespace Source77NW
{
    ...
    /// </para>
    /// <para>
    /// <see cref="SaveAsLsvFormat"/> writes non-read-only values as an LSV record;
    /// <see cref="LoadAsLsvFormat"/> sets values by matching field names to member
    ...
    public sealed class EnumVals
    {
        ...
        /// <summary>Notification raised after LoadAsLsvFormat sets its values.</summary>
        ...
        /// <summary>The values as LSV text (<see cref="SaveAsLsvFormat"/> into a builder).</summary>
        public string ToLsv()
        {
        ...
        /// </summary>
        public void SaveAsLsvFormat(TextWriter writer)
        {
            // NOTE: SaveAsLsvFormat skips ReadOnly items, LoadAsLsvFormat does not - intentional:
        ...
        /// unless addingValues.
        /// </summary>
        public bool LoadAsLsvFormat(Chars theTextAsLsv, bool addingValues = false)
        {
            if (!addingValues)
                ClearValues();

            if (LsvRecord.Parsed(ref theTextAsLsv, out LsvRecord v1))
            {
                _Context = v1.Context;
                int cursor = 0;
